...
export function start(): void {
  ...
  function onAirRow(row: {
    ...
    // Once this tab is already in the channel, a second Join action only
    // repeats the same request and crowds the live row. Keep the row and its
    // share/admin controls, but hide the action that no longer applies.
    play.hidden = row.key.startsWith("channel:") && channelOn?.id === row.key.slice("channel:".length);
