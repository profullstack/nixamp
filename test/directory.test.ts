...
test("a listing has to point somewhere a stranger can actually go", () => {
  assert.notEqual(publishable("https://nixamp.example.com/v/abc"), null);
  assert.notEqual(publishable("http://67.205.189.229:4321/v/abc"), null);

  // Reachable only from the machine that published it, so listing it is an
  // entry nobody else can ever open.
  assert.equal(publishable("http://localhost:4321/v/abc"), null);
  assert.equal(publishable("http://127.0.0.1:4321/v/abc"), null);
  assert.equal(publishable("http://[::1]:4321/v/abc"), null);
  assert.equal(publishable("http://169.254.1.1:4321/v/abc"), null);
...
test("an announcement is sanitised, and a nameless one still gets a name", () => {
  const ok = parseAnnouncement({
    name: "  Anthony's box  ",
    url: "https://nixamp.example.com/v/abc",
    tracks: 12.7,
    nowPlaying: "A Song",
  });
...
test("a heartbeat that omits the audio address does not blank it", () => {
  ...
  tick(60_000);
  const again = dir.announce(stream("https://a.example/v/abc"));
