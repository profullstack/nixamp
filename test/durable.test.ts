...
test("an ended stream is echoed as it is remembered, and dropped when it returns", () => {
  ...
  const live = dir.announce(
    { name: "Chovy", url: "https://a.example/s/1", tracks: 1, nowPlaying: "x" },
    "owner-1",
  );
...
test("a restarted process remembers the streams that had ended", () => {
  ...
  dir.seedEnded([{
    id: "s-old", code: "482917", name: "Chovy", url: "https://a.example/s/1",
...
test("seeding never overwrites what this process already knows", () => {
  let at = NOW;
  const dir = new Directory(4 * 60 * 1000, () => at, () => "482917");
  dir.announce({ name: "Fresh", url: "https://a.example/s/1", tracks: 1, nowPlaying: "" }, "o1");
  at += 5 * 60 * 1000;
  dir.list();

  const before = dir.endedByCode("482917")?.name;
  dir.seedEnded([{
    id: dir.recentlyEnded()[0]!.id, code: "482917", name: "Stale", url: "https://a.example/s/1",
